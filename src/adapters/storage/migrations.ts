 ...
 *
 * v4 -> v5 (T060): the `tombstones` store was added, so a deletion survives
...
/**
 * v4 -> v5: the `tombstones` store landed beside `decks`. A Tombstone names
 * the deleted Deck or Mix by id and holds nothing from inside it, so no
 * deck record gains, loses or changes a field — identity, like every step
 * before it. A library upgraded through this step has no Tombstones, which
 * is the truth: nothing was ever recorded as deleted before v5.
 */
